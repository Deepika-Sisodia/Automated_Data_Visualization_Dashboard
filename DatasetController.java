    ...
    if (config.type === 'LineChart') {
      return (
        <LineChart data={chartData} margin={MARGIN}>
          ...
          {tip}
          <Line type="monotone" dataKey={config.yAxis} stroke="#E74C3C" strokeWidth={2.5} dot={{ fill: '#E74C3C', r: 3 }} activeDot={{ r: 5 }} />
        </LineChart>
      );
    }
    ...
    if (config.type === 'PieChart') {
      return (
        <PieChart>
          ...
          <Pie data={chartData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={85} innerRadius={30} label={{ fontSize: 11 }}>
            {chartData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
          </Pie>
        </PieChart>
      );
